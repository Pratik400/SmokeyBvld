<!DOCTYPE html>
<html lang="en">

<head>
    <?php include './aaheader.php'; ?>
</head>

<body>
    <?php include './aanavbar.php'; ?>


    <section class="section-wishlist">
        <div class="container">

            <header class="wishlist-header">
                <strong>Items in Your Wishlist</strong>
            </header>

            <div class="row px-3 row-cols-2 row-cols-sm-3 row-cols-md-4 row-cols-lg-5">


                <div class="col">
                    <div class="filter-product">
                        <a href="./product.php">
                            <div class="sale">
                                <span> on sale</span>
                            </div>
                            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRWpjsOx8_bK4Qsc-va4esLHDP1OJ9G1LPBwg&usqp=CAU" alt="">

                            <div class="p-stars">
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="far fa-star"></i>
                                <span>(453)</span>
                            </div>
                            <div class="p-name">Pandora Ash Catcher</div>
                            <div class="p-price">$74.99 <del>$90.00</del></div>
                        </a>
                    </div>
                </div>

                <div class="col">
                    <div class="filter-product">
                        <a href="./product.php">
                            <div class="sale">
                                <span> on sale</span>
                            </div>
                            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRWpjsOx8_bK4Qsc-va4esLHDP1OJ9G1LPBwg&usqp=CAU" alt="">

                            <div class="p-stars">
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="far fa-star"></i>
                                <span>(453)</span>
                            </div>
                            <div class="p-name">Pandora Ash Catcher</div>
                            <div class="p-price">$74.99 <del>$90.00</del></div>
                        </a>
                    </div>
                </div>

                <div class="col">
                    <div class="filter-product">
                        <a href="./product.php">
                            <div class="sale">
                                <span> on sale</span>
                            </div>
                            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRWpjsOx8_bK4Qsc-va4esLHDP1OJ9G1LPBwg&usqp=CAU" alt="">

                            <div class="p-stars">
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="far fa-star"></i>
                                <span>(453)</span>
                            </div>
                            <div class="p-name">Pandora Ash Catcher</div>
                            <div class="p-price">$74.99 <del>$90.00</del></div>
                        </a>
                    </div>
                </div>

                <div class="col">
                    <div class="filter-product">
                        <a href="./product.php">
                            <div class="sale">
                                <span> on sale</span>
                            </div>
                            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRWpjsOx8_bK4Qsc-va4esLHDP1OJ9G1LPBwg&usqp=CAU" alt="">

                            <div class="p-stars">
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="far fa-star"></i>
                                <span>(453)</span>
                            </div>
                            <div class="p-name">Pandora Ash Catcher</div>
                            <div class="p-price">$74.99 <del>$90.00</del></div>
                        </a>
                    </div>
                </div>

                <div class="col">
                    <div class="filter-product">
                        <a href="./product.php">
                            <div class="sale">
                                <span> on sale</span>
                            </div>
                            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRWpjsOx8_bK4Qsc-va4esLHDP1OJ9G1LPBwg&usqp=CAU" alt="">

                            <div class="p-stars">
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="far fa-star"></i>
                                <span>(453)</span>
                            </div>
                            <div class="p-name">Pandora Ash Catcher</div>
                            <div class="p-price">$74.99 <del>$90.00</del></div>
                        </a>
                    </div>
                </div>

                <div class="col">
                    <div class="filter-product">
                        <a href="./product.php">
                            <div class="sale">
                                <span> on sale</span>
                            </div>
                            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRWpjsOx8_bK4Qsc-va4esLHDP1OJ9G1LPBwg&usqp=CAU" alt="">

                            <div class="p-stars">
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="far fa-star"></i>
                                <span>(453)</span>
                            </div>
                            <div class="p-name">Pandora Ash Catcher</div>
                            <div class="p-price">$74.99 <del>$90.00</del></div>
                        </a>
                    </div>
                </div>

                <div class="col">
                    <div class="filter-product">
                        <a href="./product.php">
                            <div class="sale">
                                <span> on sale</span>
                            </div>
                            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRWpjsOx8_bK4Qsc-va4esLHDP1OJ9G1LPBwg&usqp=CAU" alt="">

                            <div class="p-stars">
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="far fa-star"></i>
                                <span>(453)</span>
                            </div>
                            <div class="p-name">Pandora Ash Catcher</div>
                            <div class="p-price">$74.99 <del>$90.00</del></div>
                        </a>
                    </div>
                </div>

                <div class="col">
                    <div class="filter-product">
                        <a href="./product.php">
                            <div class="sale">
                                <span> on sale</span>
                            </div>
                            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRWpjsOx8_bK4Qsc-va4esLHDP1OJ9G1LPBwg&usqp=CAU" alt="">

                            <div class="p-stars">
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="far fa-star"></i>
                                <span>(453)</span>
                            </div>
                            <div class="p-name">Pandora Ash Catcher</div>
                            <div class="p-price">$74.99 <del>$90.00</del></div>
                        </a>
                    </div>
                </div>

                <div class="col">
                    <div class="filter-product">
                        <a href="./product.php">
                            <div class="sale">
                                <span> on sale</span>
                            </div>
                            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRWpjsOx8_bK4Qsc-va4esLHDP1OJ9G1LPBwg&usqp=CAU" alt="">

                            <div class="p-stars">
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="far fa-star"></i>
                                <span>(453)</span>
                            </div>
                            <div class="p-name">Pandora Ash Catcher</div>
                            <div class="p-price">$74.99 <del>$90.00</del></div>
                        </a>
                    </div>
                </div>

                <div class="col">
                    <div class="filter-product">
                        <a href="./product.php">
                            <div class="sale">
                                <span> on sale</span>
                            </div>
                            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRWpjsOx8_bK4Qsc-va4esLHDP1OJ9G1LPBwg&usqp=CAU" alt="">

                            <div class="p-stars">
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="far fa-star"></i>
                                <span>(453)</span>
                            </div>
                            <div class="p-name">Pandora Ash Catcher</div>
                            <div class="p-price">$74.99 <del>$90.00</del></div>
                        </a>
                    </div>
                </div>

                <div class="col">
                    <div class="filter-product">
                        <a href="./product.php">
                            <div class="sale">
                                <span> on sale</span>
                            </div>
                            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRWpjsOx8_bK4Qsc-va4esLHDP1OJ9G1LPBwg&usqp=CAU" alt="">

                            <div class="p-stars">
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="far fa-star"></i>
                                <span>(453)</span>
                            </div>
                            <div class="p-name">Pandora Ash Catcher</div>
                            <div class="p-price">$74.99 <del>$90.00</del></div>
                        </a>
                    </div>
                </div>

                <div class="col">
                    <div class="filter-product">
                        <a href="./product.php">
                            <div class="sale">
                                <span> on sale</span>
                            </div>
                            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRWpjsOx8_bK4Qsc-va4esLHDP1OJ9G1LPBwg&usqp=CAU" alt="">

                            <div class="p-stars">
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="far fa-star"></i>
                                <span>(453)</span>
                            </div>
                            <div class="p-name">Pandora Ash Catcher</div>
                            <div class="p-price">$74.99 <del>$90.00</del></div>
                        </a>
                    </div>
                </div>

                <div class="col">
                    <div class="filter-product">
                        <a href="./product.php">
                            <div class="sale">
                                <span> on sale</span>
                            </div>
                            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRWpjsOx8_bK4Qsc-va4esLHDP1OJ9G1LPBwg&usqp=CAU" alt="">

                            <div class="p-stars">
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="far fa-star"></i>
                                <span>(453)</span>
                            </div>
                            <div class="p-name">Pandora Ash Catcher</div>
                            <div class="p-price">$74.99 <del>$90.00</del></div>
                        </a>
                    </div>
                </div>

                <div class="col">
                    <div class="filter-product">
                        <a href="./product.php">
                            <div class="sale">
                                <span> on sale</span>
                            </div>
                            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRWpjsOx8_bK4Qsc-va4esLHDP1OJ9G1LPBwg&usqp=CAU" alt="">

                            <div class="p-stars">
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="far fa-star"></i>
                                <span>(453)</span>
                            </div>
                            <div class="p-name">Pandora Ash Catcher</div>
                            <div class="p-price">$74.99 <del>$90.00</del></div>
                        </a>
                    </div>
                </div>

                <div class="col">
                    <div class="filter-product">
                        <a href="./product.php">
                            <div class="sale">
                                <span> on sale</span>
                            </div>
                            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRWpjsOx8_bK4Qsc-va4esLHDP1OJ9G1LPBwg&usqp=CAU" alt="">

                            <div class="p-stars">
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="far fa-star"></i>
                                <span>(453)</span>
                            </div>
                            <div class="p-name">Pandora Ash Catcher</div>
                            <div class="p-price">$74.99 <del>$90.00</del></div>
                        </a>
                    </div>
                </div>

                <div class="col">
                    <div class="filter-product">
                        <a href="./product.php">
                            <div class="sale">
                                <span> on sale</span>
                            </div>
                            <img src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcRWpjsOx8_bK4Qsc-va4esLHDP1OJ9G1LPBwg&usqp=CAU" alt="">

                            <div class="p-stars">
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="fa fa-star"></i>
                                <i class="far fa-star"></i>
                                <span>(453)</span>
                            </div>
                            <div class="p-name">Pandora Ash Catcher</div>
                            <div class="p-price">$74.99 <del>$90.00</del></div>
                        </a>
                    </div>
                </div>



                <div class="col-12">
                    <div class="cart-footer">
                        <a href="/" class="btn btn-light">Continue Shopping</a>
                        <a href="./cart.php" class="btn btn-black-outline">View Cart</a>
                    </div>
                </div>

            </div>

        </div>
    </section>

    <?php include './aafooter.php'; ?>


    <!-- page specific js -->

    <script>

    </script>


</body>

</html>